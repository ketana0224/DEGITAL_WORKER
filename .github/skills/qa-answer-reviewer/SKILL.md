...
各回答に対して **5 つの必須検査 + 品質ルール検査** を実施します。
詳細は [レビュー基準](./references/review-criteria.md) および
[回答品質ルール](../qa-research-responder/references/quality-rules.md) を参照してください。
...
| 5 | **推測混入** | `[推測]` `⚠️` マーク / 断定的だが出典がない記述の検出 |

**品質ルール検査 (必須)**:

| # | 検査項目 | 検出方法 | 重大度 |
|---|----------|----------|--------|
| 6 | **事実と推測の分離** | `✅ 事実` / `⚠️ 推測` セクション分けの有無 | 🟡 Minor |
| 7 | **推測の断定** | 推測内容が「〜です」「〜である」と断定形 | 🔴 Major |
| 8 | **出典なし断定** | 出典が紐づかない断定的記述 | 🔴 Major |
| 9 | **「である調」「だ調」混入** | 文末「〜だ」「〜である」「〜せよ」等 | 🟡 Minor |
| 10 | **「不明」の推測補完** | `[不明]` とせず推測で穴埋めしている | 🔴 Major |
...
- **外部 URL の取得** は 1 リクエスト 5 秒以内のタイムアウトに設定してください。
- **機密回答** をレビューする場合は外部 fetch を行わず構文・引用整合性のみチェックしてください。
- レビュアの推測も `[レビュア推測]` とマークしてください。
- 元ファイルは **直接書き換えないでください**。修正は提案ベースで、適用は別アクションで行います。
- レビューレポート自体も **ですます調** で記述してください。
- 共通の品質ルールは [回答品質ルール](../qa-research-responder/references/quality-rules.md) を参照してください。
